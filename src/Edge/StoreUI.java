...
public class StoreUI {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			System.out.println(system.barrier.automaticControl ? "   2- Disable automatic control"
			...
			case '1': {
				System.out.println("Manager goes to desktop. Enter AdminPass:");
				...
				if(store.verifyManager(adminPass)) {
					...
					System.out.println("Incorrect Adminpass. Returning to Main Menu");
				break;
			}
			...
			case '2': {
				...
				System.out.println(
						system.barrier.automaticControl ? "Automatic control enabled" : "Automatic control disabled");
			...
			case '6': {
				...
				// Checking if there are no customers in the store nobody can leave otherwise
				// deducts a person
				if (system.barrier.automaticControl) {
				...
				// manual control where the doors are manually opened or shut and there might be
				// 0 customers in the store if otherwise then a customer leaves
				if (system.viewCount() <= 0) {
					System.out.println("Nobody left the store because nobody is in the store!");
					break;
				}
